 ...
 * Author: JonathanS
 *
 * Created on 29 de julio de 2014, 01:00 PM
 */
...
#define MPU6050_FIFO_EN_ADDR            0x23   // R/W
...
int MPU6050_read(int start, unsigned char *buffer, int size)
{
    ...
    i2cData[1] = start;              // EEPROM location to read (high address byte)
    for (i=0;i<size;i++)
    {
        i2cData[2+i] =0x00;
    }
    ...
    do
    {
        if(Success)
        {
            ...
            else
            {
                ...
                if (size>1)
                {
                    I2CAcknowledgeByte(MPU6050_I2C_BUS, TRUE);
                }
                *buffer = I2CGetByte(MPU6050_I2C_BUS);
                if (size>1)
                {
                    while(!I2CAcknowledgeHasCompleted(MPU6050_I2C_BUS));
                }
                buffer++;
            }
        }
...
{   int i;
    ...
    i2cData[0] = SlaveAddress.byte;
    i2cData[1] = start;              // EEPROM location to program (high address byte)
...
void GYROloop(char data[], int an)
{
    int error;
    unsigned char c0;
    unsigned char c1;
    ...
    //------------------------------------------------------
    // Inicialización... Esto se supone que va en el Setup()
    //------------------------------------------------------

    error = MPU6050_read (MPU6050_ACCEL_XOUT_H, &c0, 1);
    error = MPU6050_read (MPU6050_ACCEL_XOUT_L, &c1, 1);
    accel_t_gyro.value.x_accel=((int)c0<<8)+c1;
    error = MPU6050_read (MPU6050_ACCEL_ZOUT_H, &c0, 1);
    error = MPU6050_read (MPU6050_ACCEL_ZOUT_L, &c1, 1);
    accel_t_gyro.value.z_accel=((int)c0<<8)+c1;
    error = MPU6050_read (MPU6050_ACCEL_YOUT_H, &c0, 1);
    error = MPU6050_read (MPU6050_ACCEL_YOUT_L, &c1, 1);
    accel_t_gyro.value.y_accel=((int)c0<<8)+c1;

    error = MPU6050_read ( MPU6050_GYRO_XOUT_H  , &c0, 1);
    error = MPU6050_read ( MPU6050_GYRO_XOUT_L  , &c1, 1);
    accel_t_gyro.value.x_gyro=((int)c0<<8)+c1;
    error = MPU6050_read ( MPU6050_GYRO_ZOUT_H  , &c0, 1);
    error = MPU6050_read ( MPU6050_GYRO_ZOUT_L  , &c1, 1);
    accel_t_gyro.value.z_gyro=((int)c0<<8)+c1;
    error = MPU6050_read ( MPU6050_GYRO_YOUT_H  , &c0, 1);
    error = MPU6050_read ( MPU6050_GYRO_YOUT_L  , &c1, 1);
    accel_t_gyro.value.y_gyro=((int)c0<<8)+c1;

    roll  = atan2(accel_t_gyro.value.y_accel, accel_t_gyro.value.z_accel) * 57.2958; //57.28***
    pitch = atan(-accel_t_gyro.value.x_accel / sqrt(accel_t_gyro.value.y_accel * accel_t_gyro.value.y_accel + accel_t_gyro.value.z_accel * accel_t_gyro.value.z_accel)) * 57.2958;

    // Inicializamos el Kalman Filter
    KalmanXY_Inicializacion();
    KalmanX_setAngle(roll); // Set starting angle
    KalmanY_setAngle(pitch);
    gyroXangle = roll;
    gyroYangle = pitch;
    compAngleX = roll;
    compAngleY = pitch;
    ...
    asm volatile("mfc0   %0, $9" : "=r"(new_period));
    // Mult * 12.5 ns y no se divide entre nada

    int DTi=(new_period - timer) ;
    dT = (double)(new_period - timer) * (12.5e-9); // Calculate delta time
    asm volatile("mfc0   %0, $9" : "=r"(timer));
    
    roll  = atan2(accel_t_gyro.value.y_accel, accel_t_gyro.value.z_accel) * 57.2958; //57.28***
    pitch = atan2(-accel_t_gyro.value.x_accel, accel_t_gyro.value.z_accel) * 57.2958;
...
//Confirmado
    ...
    if (Dx>256)
    {
        ...
        PORTDbits.RD2 = PORTDbits.RD2 ^1;
    }

    gyroXrate = accel_t_gyro.value.x_gyro / 131.0; // Convert to deg/s
    gyroYrate = accel_t_gyro.value.y_gyro / 131.0; // Convert to deg/s
    
    // This fixes the transition problem when the accelerometer angle jumps between -180 and 180 degrees
    if ((roll < -90 && kalAngleX > 90) || (roll > 90 && kalAngleX < -90))
    {
        KalmanX_setAngle(roll);
        compAngleX = roll;
        kalAngleX = roll;
        gyroXangle = roll;
    }
    ...
    // Al final del filtro... ¿Qué es lo que tengo?
